...
SCRIPT_PROMPT = """நீங்கள் "ஆலய மணி" YouTube channel-க்கான expert Tamil devotional story writer.
நீங்கள் ஒரு கதை சொல்லிகிறீர்கள் — facts அல்ல, emotions ஆம். Podcast-style, friend-to-friend.
...
━━━━━━━━━━━━━━━━━━━━━━━━━
STORY STRUCTURE (கட்டாயம்):

பகுதி 1 — HOOK (0-15 வினாடி):
  கதையின் மிகவும் ஆச்சர்யமான தருணத்தில் DROP செய்யுங்கள்.
  ❌ "வணக்கம் நண்பர்களே", "இன்று நாம் பார்க்கப்போவது" — தொடங்க வேண்டாம்
  ✅ "அவர் மலை உச்சியில் தனியாக நின்றார். கையில் வேல் — ஆனால் உள்ளம் நடுங்கியது."
  ✅ ஒரு NUMBER, ஒரு PLACE, ஒரு MOMENT உடன் தொடங்குங்கள்.
  [PAUSE_LONG] hook reveal-க்கு பிறகு

பகுதி 2 — STORY BODY (80% of script):
  இந்த 3 அடுக்கு structure கட்டாயம்:

  அடுக்கு A — யார்? எங்கு? எப்போது? (குறிப்பிட்ட characters, places, years)
    - ஒரு specific scene-ல் வைத்து விவரியுங்கள்
    - காட்சி போல் — "அவர் கண்களில் நீர்", "அந்த கல் மண்டபத்தில்"
    [PAUSE_SHORT] ஒவ்வொரு முக்கிய fact-க்கும் பிறகு

  அடுக்கு B — conflict அல்லது மர்மம் (ஏன் இது special?)
    - கதையின் திருப்புமுனை
    - "ஆனால் யாரும் எதிர்பார்க்காத ஒன்று நடந்தது..."
    - ~2:30 mark-ல் retention hook: "இதில் ஒரு ரகசியம் இருக்கிறது — கோவிலில் இன்னும் காண்பிக்கிறார்கள்"
    [PAUSE_MED] கேள்விக்கு முன்

  அடுக்கு C — resolution + deeper meaning
    - கதை முடிவு + spiritual significance
    - viewer-க்கு இது என்ன பொருள்?

பகுதி 3 — EMOTIONAL CLOSE + CTA (கடைசி 20%):
  - ஒரு personal reflection வரி: "நாமும் அந்த பக்தனைப் போல்..."
  - "இந்த கதை உங்களை தொட்டிருந்தால் — லைக் செய்யுங்கள்"
  - "ஆலய மணி சேனலில் தினமும் இதுபோன்ற கதைகள் — சந்தா செய்யுங்கள், மணி சின்னம் அழுத்துங்கள்"

━━━━━━━━━━━━━━━━━━━━━━━━━
WRITING RULES (மீறக்கூடாதவை):
1. 100% தமிழ் — ஒரே ஒரு ஆங்கில வார்த்தை கூட வேண்டாம்
   "subscribe" → "சந்தா செய்யுங்கள்" | "like" → "லைக்" | "channel" → "சேனல்"
   "temple" → "கோவில்" | "story" → "கதை" | "science" → "விஞ்ஞானம்"
2. தொடர்ந்த பேச்சு மட்டும் — bullet points, headers, numbers, markdown வேண்டாம்
3. {target_words} தமிழ் வார்த்தைகள்
4. ஒவ்வொரு 40 வினாடிக்கும் — ஒரு புதிய fact, twist, அல்லது emotion
5. SPECIFIC-ஆக இருங்கள் — "ஒரு கோவில்" வேண்டாம், "திருவண்ணாமலை அண்ணாமலையார் கோவில்" என்று சொல்லுங்கள்
6. Emotions: ஆச்சர்யம் → சந்தேகம் → பயம் → நம்பிக்கை → triumph → அமைதி"""
...
def _resolve_pexels_video_queries(deity_key):
    if deity_key in DEITY_PEXELS_QUERIES:
        return list(DEITY_PEXELS_QUERIES[deity_key])
    return list(GENERIC_PEXELS_QUERIES)
...
def fetch_pexels_videos(deity_key, output_dir, count=5, orientation="landscape"):
    ...
    os.makedirs(output_dir, exist_ok=True)
    queries = _resolve_pexels_video_queries(deity_key)
    import datetime as _dt
    week_seed = int(_dt.datetime.now().strftime("%Y%W"))
    _rng = __import__("random").Random(week_seed)
    _rng.shuffle(queries)
...
def build_stock_video(scenes, stock_videos, fallback_images, audio_path, width, height, output_path, output_name):
    if not scenes or not fallback_images:
        return False
    ...
    ], timeout=max(900, int(get_dur_float(audio_path) * 2)))
...
def create_video(script_text, images_input, output_name, bgm, bgm_vol=0.18,
    ...
    if USE_STOCK_VIDEO and PEXELS_API_KEY:
        ...
        if stock_video_rendered:
            log(f"  ✅ Stock video rendered ({len(long_scenes)} scenes, {time.time()-t0:.0f}s)")
        else:
            log("  ⚠️ Stock video failed — falling back to Ken Burns")

    if not stock_video_rendered:
        log("  Ken Burns + slideshow mode...")
        fps = 25
...
def daemon_mode():
    ...
    print(f"Stock video mode: {USE_STOCK_VIDEO and bool(PEXELS_API_KEY)}")
